...
for date in range(len(files)):
	if 'tactical' in files[date]:
		...
		outputFile = '~/Desktop/meterFilter/filtered.Metered.AAL' 
		...
		for flight in range(len(df0['gufi'])):
			...
			if callsign in ['AAL', 'ASQ', 'AWI', 'ENY', 'JIA', 'PDT', 'RPA']:
				...
				if terminal in ['B','C','D','E']:
					if str(df0['Held_While_Metering_On_Scheduled_Runway'][flight]) == 'True':
						if str(df0['Held_With_Non_Zero_Advisory'][flight]) == 'True':
							if df0['Passback_Hold_When_Put_On_Hold'][flight] < 0:
								df0['Passback_Hold_When_Put_On_Hold'][flight] = df0['Total_Gate_Hold_When_Put_On_Hold'][flight]
								df0['Passback_Gate_Hold_At_Ready'][flight] = df0['Total_Gate_Hold_When_Put_On_Hold'][flight]
								print('NEGATIVE PASS BACK HOLD')
								print(flight)
								print(df0['gufi'][flight])
